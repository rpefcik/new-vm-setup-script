...
4. insert your credentials when prompted
    -  you can find docker credentials on your local machine `cat ~/.git-credentials .gitconfig`
    -  docker credentials `cat ~/.docker/config.json`
        -  copy the hash string in "auth" and decode it with `echo <auth-string> | base64 -d` - you'll get `<docker_username>@<docker_password>`
